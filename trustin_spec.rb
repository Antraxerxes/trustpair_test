  ...
  describe "#update_score()" do
    ...
    context "when the evaluation type is 'VAT'" do
      ...
      context "when the <state> is favorable" do
        ...
        it "decreases the <score> of 3" do
          expect(evaluations.first.score).to eq(25)
        end
      end

      context "when the <state> is 'unconfirmed' AND the <reason> is 'ongoing_database_update'" do
        let(:evaluations) { [Evaluation.new(type: "VAT", value: "IE6388047V", score: 42, state: "unconfirmed", reason: "ongoing_database_update")] }
      ...
      context "with a <score> equal to 0" do
        let(:evaluations) { [Evaluation.new(type: "VAT", value: "LU26375245", score: 0, state: "favorable", reason: "company_opened")] }
